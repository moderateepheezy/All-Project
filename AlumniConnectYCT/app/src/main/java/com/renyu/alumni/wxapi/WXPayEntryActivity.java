...
public class WXPayEntryActivity extends Activity implements IWXAPIEventHandler {
	...
	public void onResp(BaseResp arg0) {
		...
		if (arg0.getType() == ConstantsAPI.COMMAND_PAY_BY_WX) {
			if(arg0.errCode==0) {
				CommonUtils.showCustomToast(WXPayEntryActivity.this, "支付成功", false);
				Intent intent=new Intent(WXPayEntryActivity.this, DonationMessageActivity.class);
				startActivity(intent);
			}
			else if(arg0.errCode==-1) {
				CommonUtils.showCustomToast(WXPayEntryActivity.this, "支付失败", false);
			}
			else if(arg0.errCode==-2) {
				CommonUtils.showCustomToast(WXPayEntryActivity.this, "支付取消", false);
			}
			else {
				CommonUtils.showCustomToast(WXPayEntryActivity.this, "支付失败", false);
			}
